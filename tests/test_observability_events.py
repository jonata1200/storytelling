...
from app.config.settings import DEFAULT_META_TEXT_MODEL, Settings
...
def test_redact_secrets_masks_keys_and_bearer_tokens() -> None:
    text = "Authorization: Bearer sk-vibes-secret-token api_key=abc123"

    redacted = redact_secrets(text)

    assert "sk-vibes-secret-token" not in redacted
...
def test_provider_channel_readiness_reports_new_provider_components() -> None:
    settings = Settings(
        ...
        meta_default_model=DEFAULT_META_TEXT_MODEL,
    ...
    assert text.details["provider"] == "meta"
...
def test_provider_channel_readiness_reports_missing_meta_key() -> None:
    settings = Settings(
        ai_provider="meta",
        text_provider="meta",
        meta_api_key=None,
    ...
    assert "META_API_KEY" in text.message


def test_provider_channel_readiness_reports_meta() -> None:
    settings = Settings(
        ai_provider="meta",
        text_provider="meta",
        text_provider_fallbacks="",
    ...
    assert text.status == "ready"
    assert text.details["provider"] == "meta"
    assert text.details["model"] == DEFAULT_META_TEXT_MODEL
    assert text.details["fallbacks"] == ""
...
async def test_emit_project_event_persists_redacted_correlation_context(
    ...
    try:
        event = await emit_project_event(
            ...
            OperationalEventCreate(
                ...
                message="erro com sk-vibes-secret-token",
    ...
    assert "sk-vibes-secret-token" not in event.message
    ...
    assert record.__dict__["provider"] == "meta"
