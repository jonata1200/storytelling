...
@pytest.mark.unit
def test_running_reclaim_after_is_longer_than_pending_redispatch():
    assert RUNNING_JOB_RECLAIM_AFTER > PENDING_JOB_REDISPATCH_AFTER
    assert RUNNING_JOB_RECLAIM_AFTER >= timedelta(minutes=10)


@pytest.mark.unit
def test_pending_job_is_stale_after_2_minutes():
    now = datetime.now(UTC)
    job = _make_job(updated_at=now - timedelta(minutes=3))
    assert pending_job_is_stale(job, now)


@pytest.mark.unit
def test_pending_job_not_stale_within_2_minutes():
    now = datetime.now(UTC)
    job = _make_job(updated_at=now - timedelta(seconds=30))
    assert not pending_job_is_stale(job, now)


@pytest.mark.unit
def test_running_job_not_stale_within_reclaim_window():
    now = datetime.now(UTC)
    job = _make_job(status=GenerationJobStatus.RUNNING, updated_at=now - timedelta(minutes=5))
    assert not _job_stale_after_window(job, RUNNING_JOB_RECLAIM_AFTER, now)


@pytest.mark.unit
def test_running_job_stale_after_reclaim_window():
    now = datetime.now(UTC)
    job = _make_job(
        status=GenerationJobStatus.RUNNING,
        updated_at=now - timedelta(minutes=20),
    )
    assert _job_stale_after_window(job, RUNNING_JOB_RECLAIM_AFTER, now)


@pytest.mark.unit
def test_job_can_run_pending():
    job = _make_job(status=GenerationJobStatus.PENDING)
    assert project_job_can_run(job)


@pytest.mark.unit
def test_job_cannot_run_succeeded():
    job = _make_job(status=GenerationJobStatus.SUCCEEDED)
    assert not project_job_can_run(job)


@pytest.mark.unit
def test_job_cannot_run_cancelled():
    job = _make_job(status=GenerationJobStatus.CANCELLED)
    assert not project_job_can_run(job)


@pytest.mark.unit
def test_job_cannot_run_failed_at_max_attempts():
    job = _make_job(
...
@pytest.mark.unit
def test_job_can_run_failed_under_max_attempts():
    job = _make_job(
        status=GenerationJobStatus.FAILED,
        attempts=1,
        max_attempts=3,
...
@pytest.mark.unit
def test_job_stale_with_none_updated_at():
    job = _make_job(updated_at=None)
    assert _job_stale_after_window(job, timedelta(minutes=1))


@pytest.mark.asyncio
@pytest.mark.unit
async def test_background_task_is_tracked_and_cleaned_up():
