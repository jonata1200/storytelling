...
def _request(storage: Path) -> VideoGenerationRequest:
    return VideoGenerationRequest(
        ...
        aspect_ratio="9:16",
        frame_images=[
            VideoImageInput(url="data:image/png;base64,AA==", frame_type="first_frame"),
            VideoImageInput(url="data:image/png;base64,Ag==", frame_type="last_frame"),
        ],
...
def test_vibes_request_mapping(configured: Path) -> None:
    ...
    assert payload["output_duration_seconds"] == 8
    assert payload["aspect_ratio"] == "portrait"
    assert payload["first_frame"] == "data:image/png;base64,AA=="
    assert payload["final_frame"] == "data:image/png;base64,Ag=="
    assert payload["ingredients"] == [{"id": "ing-1", "type": "character", "reference_id": "ref-1"}]


def test_vibes_request_mapping_includes_project_key(configured: Path) -> None:
    request = _request(configured).model_copy(update={"project_key": "proj-1"})
    payload = map_vibes_request(request)
    assert payload["projectKey"] == "proj-1"


def test_vibes_request_mapping_defaults_to_null_project_key(configured: Path) -> None:
    payload = map_vibes_request(_request(configured))
    assert payload["projectKey"] is None
...
async def test_vibes_contract_submit_poll_download(configured: Path) -> None:
    ...
    # O flag de áudio foi removido: o Vibes gera vídeo sem áudio e não há
    # controle de som na tela de geração (verificado empiricamente).
    assert "audio" not in backend.payload
