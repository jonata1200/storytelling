...
def test_ai_action_sync_skips_initial_script_queued_chat_message(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    project_id = uuid4()
    storage: dict[str, Any] = {}
...
def test_visual_library_cards_ready_when_any_card_type_exists() -> None:
    assert pages._visual_library_cards_ready(
        {"characters": [object()], "locations": [object()]}
    )
    assert pages._visual_library_cards_ready({"characters": [object()], "locations": []})
    assert pages._visual_library_cards_ready({"characters": [], "locations": [object()]})
    assert not pages._visual_library_cards_ready({"characters": [], "locations": []})


def test_visual_prompts_need_generation_before_visual_targets_exist() -> None:
    assert pages._visual_prompts_need_generation({"characters": [], "locations": []})
...
def test_visual_prompts_need_generation_when_any_prompt_is_missing() -> None:
    ...
    assert pages._visual_prompts_need_generation(
        {"characters": [], "locations": [ready, missing]}
    )
...
def test_visual_batch_requests_include_only_required_missing_views() -> None:
    ...
    assert requests == [("location", location_id, ["establishing"])]
...
def test_storyboard_section_waits_for_all_visual_references() -> None:
    ...
    allowed, reason = pages._workspace_section_access("video", counts)
...
def test_finalization_requires_three_generated_segment_videos() -> None:
    counts: dict[str, int] = {
        ...
        "locations": 1,
        "visual_refs": 6,
        "continuous_video_segments": 5,
        "continuous_video_generated_segments": 2,
    }

    allowed, reason = pages._workspace_section_access("finalization", counts)
    assert allowed is False
    assert "3 vídeos de segmentos" in reason

    counts["continuous_video_generated_segments"] = 3
    allowed, reason = pages._workspace_section_access("finalization", counts)
    assert allowed is False
    assert "todos os segmentos" in reason

    counts["continuous_video_generated_segments"] = 5
    allowed, reason = pages._workspace_section_access("finalization", counts)
    assert allowed is True
    assert reason == ""
