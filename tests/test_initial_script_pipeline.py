...
@pytest.mark.asyncio
async def test_dashboard_creation_registers_prompt_as_idea_before_hook_selection(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, Any] = {}
...
def test_script_revision_size_targets_round_odd_explicit_seconds_for_video_clips() -> None:
    ...
    assert duration == 200
    assert words == 600
    assert "200s" in guidance


def test_video_package_total_duration_rounds_odd_values() -> None:
    assert storytelling_service._video_package_total_duration_seconds(195) == 200
    assert storytelling_service._video_package_total_duration_seconds(196) == 200
