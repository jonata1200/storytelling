...
def test_default_generation_templates_with_json_examples_compile() -> None:
    ...
    assert "exatamente 8s" in compiled["generate_scenes_and_shots"]
...
def test_text_generation_timeout_budget_stays_bounded_for_ui_flows() -> None:
    # Modelos cloud levam 2-5+ min por roteiro (medido 08/09/2026: 63s-307s);
    # o teto garante que a UI sempre recupera o controle em tempo finito, mas
    # é alto o suficiente para não abortar tentativas válidas.
    ...
    assert generation_service.TASK_TIMEOUT_SECONDS["generate_story_hooks"] <= 240
    for task in (
        "generate_script",
        ...
        assert generation_service.TASK_TIMEOUT_SECONDS[task] <= 480
