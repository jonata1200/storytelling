...
def _record_deleted_bridge_job_ids(target: list[str], job_ids: list[str]) -> int:
    target.extend(job_ids)
    return len(job_ids)
...
class _FakeHardDeleteSession:
    ...
    async def execute(self, statement: Any, params: Any | None = None) -> Any:
        ...
        # Consulta exata do id dos projetos-alvo (não as subqueries que a
        # referenciam via "IN (SELECT id FROM tmp_target_projects)").
        if sql.strip() == "SELECT id FROM tmp_target_projects":
...
async def test_hard_delete_project_removes_project_graph_physically(
    ...
    deleted_storage_uris: list[str] = []
    deleted_project_ids: list[UUID] = []
    deleted_bridge_job_ids: list[str] = []
    monkeypatch.setattr(
        project_service,
        "delete_local_storage_files",
        lambda storage_uris: _record_deleted_storage_uris(deleted_storage_uris, storage_uris),
    ...
    monkeypatch.setattr(
        ...
        lambda job_ids: _record_deleted_bridge_job_ids(deleted_bridge_job_ids, job_ids),
    )
    ...
    qa_results_delete_index = next(
        index for index, sql in enumerate(session.executed) if "DELETE FROM qa_results" in sql
    )
    assert qa_results_delete_index < continuous_segments_delete_index
