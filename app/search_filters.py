...
def safe_datetime(value: object) -> datetime | None:
    ...
    if isinstance(value, datetime):
        ...
        text = str(value)
        # Replace only trailing Z to avoid corrupting Z in other positions.
        if text.endswith("Z"):
            text = text[:-1] + "+00:00"
        try:
...
def project_matches_updated_period(
    ...
    # Reject future-dated projects (clock skew or bad data) from "last N days".
    if updated_at > reference:
        return False
    return reference - updated_at <= timedelta(days=days)
