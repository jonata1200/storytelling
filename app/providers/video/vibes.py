...
from app.providers.video.vibes_browser import (
    UnconfiguredVibesBrowserBackend,
    VibesBrowserBackend,
)
...
class VibesVideoProvider:
    ...
    def __init__(self, browser_backend: VibesBrowserBackend | None = None) -> None:
        self._browser = browser_backend or UnconfiguredVibesBrowserBackend()

    def _ensure_browser_mode(self) -> None:
        mode = provider_integration_mode(get_settings(), self.provider_name, "video")
        if mode == "api":
            raise RuntimeError(
