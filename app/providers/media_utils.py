...
# SEC-05: política de execução de subprocessos (ruff S603).
# Todos os subprocess.run deste módulo executam SOMENTE o binário ffmpeg
# resolvido por resolve_ffmpeg_path, com:
#   - shell=False sempre (argumentos passados como lista);
#   - argumentos compostos exclusivamente por flags fixas de ffmpeg e por
#     caminhos internos resolvidos via resolve_storage_path (que exige que o
#     arquivo exista dentro do storage root);
#   - timeout explícito em todas as chamadas.
# Não há execução de input remoto. FFMPEG_PATH é configurável pelo operador
# local por design (equivalente a conceder acesso à máquina). Ao adicionar um
# novo subprocess aqui, mantenha essas invariantes.
...
def resolve_ffmpeg_path(configured_path: object = None) -> str | None:
    ...
    if configured:
        ...
        if discovered:
            return discovered
        logger.warning(
            "ffmpeg_path configurado não resolveu para um executável: %r",
            configured,
        )
...
def normalize_video_duration(
    ...
    *,
    ...
    """Recorta ou prolonga o último frame para entregar um clipe de duração exata."""
    resolved = video_path.resolve(strict=True)
    current_duration = get_video_duration(resolved)
    if abs(current_duration - target_seconds) <= tolerance_seconds:
        return current_duration
    ffmpeg_path = resolve_ffmpeg_path(getattr(get_settings(), "ffmpeg_path", ""))
    if not ffmpeg_path:
        raise RuntimeError("ffmpeg não encontrado para normalizar o vídeo para 8 segundos")
    temporary = resolved.with_name(f".{resolved.stem}-normalized.mp4")
    command = [
        ffmpeg_path,
        "-y",
        "-hide_banner",
    ...
    try:
        ...
        normalized_duration = get_video_duration(temporary)
        if abs(normalized_duration - target_seconds) > tolerance_seconds:
            raise RuntimeError(
                f"normalização produziu {normalized_duration:.2f}s em vez de 8s"
            )
        temporary.replace(resolved)
        return normalized_duration
    except (OSError, subprocess.SubprocessError) as exc:
        raise RuntimeError(f"não foi possível normalizar o vídeo para 8 segundos: {exc}") from exc
    finally:
        if temporary.exists():
            temporary.unlink()
