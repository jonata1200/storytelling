...
class OmniRouteSpeechProvider:
    ...
    def _synthesize(self, request: SpeechRequest) -> SpeechResult:
        ...
        model = request.model or settings.omniroute_speech_model or settings.speech_model
        if not model:
            raise RuntimeError("OMNIROUTE_SPEECH_MODEL ou SPEECH_MODEL não configurado")

        body = self._speech_request_body(request, model, settings.speech_voice)
        audio_bytes = self._post_speech(settings.omniroute_base_url, api_key, body)
    ...
    def _post_speech(
        ...
        try:
            ...
            raise RuntimeError(
                f"OmniRoute Speech HTTP {exc.code}: {redact_secrets(detail)}"
            ) from exc
        except urllib.error.URLError as exc:
            raise RuntimeError(
                f"OmniRoute Speech network error: {redact_secrets(str(exc.reason))}"
            ) from exc
