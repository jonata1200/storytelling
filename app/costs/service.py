...
def _operation_cost_policy(
    ...
    if normalized_provider == "mock":
        default_cost = DEFAULT_OPERATION_COSTS_USD.get(
            normalized_operation, ("unit", Decimal("0.000000"))
        )
        return default_cost[0], Decimal("0.000000")
    if normalized_model:
...
def _budget_decimal(value: object) -> Decimal | None:
    ...
    try:
        ...
        logger.warning("budget_decimal_parse_failed value=%r", value)
        return None
    if parsed < 0:
        return None
...
async def project_cost_for_budget(
    ...
    """Compute net project cost via SQL aggregation instead of loading all rows.

    Uses a single query with CASE to sign credits as negative, then SUM.
    Optionally filters by stage via JSON metadata lookup.
    """
    normalized_stage = stage.strip().lower() if stage else None
    signed_total = case(
        (CostEntry.entry_type == CostEntryType.CREDIT, -CostEntry.total_cost),
        else_=CostEntry.total_cost,
    )
    statement = select(func.coalesce(func.sum(signed_total), 0)).where(
        CostEntry.project_id == project_id
    )
    if normalized_stage is not None:
        # Filter by stage stored in metadata_json->>'stage' or operation fallback.
        statement = statement.where(
            func.coalesce(
                CostEntry.metadata_json["stage"].astext,
                CostEntry.operation,
            ).ilike(normalized_stage)
        )
...
async def assert_project_budget_allows(
    ...
    if not check.allowed:
        ...
        # Flush already done in emit_project_event, but ensure it's visible.
        await session.flush()
...
def _add_to_breakdown(item: CostBreakdownItemRead, entry: CostEntry) -> None:
    ...
    # Net total: when actuals exist for a key, estimates are superseded.
    # total_cost = max(actual, estimated) - credit, avoiding double-counting.
    item.total_cost = _money(max(item.actual_cost, item.estimated_cost) - item.credit)
...
async def project_cost_summary(
    ...
    """Build cost summary using a single DB query with conditional aggregation.

    Instead of loading all CostEntry rows into memory, uses SUM(CASE) to
    compute estimated/actual/credit totals in one round-trip. Breakdowns
    still iterate entries (needed for per-key grouping) but the top-level
    ...
    return ProjectCostSummaryRead(
        ...
        total_cost=_money(max(actual, estimated) - credit),
