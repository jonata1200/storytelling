...
def _local_asset_path(storage_uri: str) -> Path:
    ...
    # SEC-02: defesa em profundidade — a contenção já é feita por
    # resolve_local_asset_path; re-verificar aqui garante que um storage_uri
    # arbitrário gravado no banco não vire leitura de arquivo arbitrário.
    storage_root = get_settings().local_storage_path.resolve()
    try:
        path.resolve().relative_to(storage_root)
    except (OSError, ValueError) as exc:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Asset file not found"
        ) from exc
    return path
