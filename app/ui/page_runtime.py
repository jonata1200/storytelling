...
def _notify_ai_action_failure_once(project_id: UUID, summary: dict[str, Any]) -> None:
    ...
    message = str(ai_action.get("message") or "A IA não conseguiu concluir a etapa.")
    error = str(ai_action.get("error") or "").strip()
    notify_key = f"{project_id}:{action}:{updated_at}"
    nicegui_app = _ui_pages().nicegui_app
    store = nicegui_app.storage.user.setdefault("notified_ai_action_failures", [])
    notified = [str(item) for item in store if isinstance(item, str)]
    if notify_key in notified:
        return
    notified.append(notify_key)
    nicegui_app.storage.user["notified_ai_action_failures"] = notified[-80:]
    try:
        from nicegui import ui

        ui.notify(
            message if not error else f"{message} {error}",
            color="negative",
            timeout=10000,
            close_button=True,
        )
    except (AssertionError, RuntimeError):
        # Contexto de UI removido (usuário navegou); nada a notificar.
        return
...
def _ai_action_is_stale(action: dict[str, Any], max_age_seconds: int = 300) -> bool:
    """Considera stale só depois de 5min sem NENHUM progresso reportado.

    A geração de roteiro em modelos cloud leva 2-5+ min sem atualizações
    intermediárias; com 120s a UI re-agendava a etapa em plena execução válida
    (dupla geração e confusão de estado).
    """
