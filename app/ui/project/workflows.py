...
def schedule_initial_script_generation(
    ...
    existing = _INITIAL_SCRIPT_TASKS.get(project_id)
    if existing is not None and not existing.done():
        # Já existe uma geração em andamento para este projeto. Não criar uma
        # segunda execução concorrente (evita roteiro gerado em duplicidade).
        return existing
...
async def _sync_project_title_from_story(
    ...
    *,
    ...
    clean_title = standardize_title_case(title)
    if not clean_title:
        return
...
async def _generate_initial_script_in_background(
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            existing_script = await _latest(session, Script, project_id)
