...
from app.visual_bible.manual_references import load_visual_exclusions
...
def project_counts_statement(project_id: UUID) -> Any:
    ...
    return union_all(
        ...
        select(
            literal("visual_refs_ready").label("label"),
            func.count().label("value"),
        )
        .select_from(VisualReference)
        .join(Artifact, VisualReference.artifact_id == Artifact.id)
        .where(
            VisualReference.project_id == project_id,
            Artifact.status.notin_(INACTIVE_DERIVED_STATUSES),
            or_(
                (VisualReference.target_kind == "character")
                & (VisualReference.view_type == "full_body"),
                (VisualReference.target_kind == "location")
                & (VisualReference.view_type == "establishing"),
            ),
        ),
        _plain("clips", VideoClip),
        _plain("continuous_video_segments", ContinuousVideoSegment),
        select(
            literal("storyboard_ready_segments").label("label"),
            func.count().label("value"),
        )
        .select_from(ContinuousVideoSegment)
        .where(
            ContinuousVideoSegment.project_id == project_id,
            ContinuousVideoSegment.source_frame_asset_id.isnot(None),
            ContinuousVideoSegment.final_frame_asset_id.isnot(None),
        ),
        select(
            literal("storyboard_approved_segments").label("label"),
            func.count().label("value"),
        )
        .select_from(ContinuousVideoSegment)
        .where(
            ...
            ContinuousVideoSegment.source_frame_asset_id.isnot(None),
            ContinuousVideoSegment.final_frame_asset_id.isnot(None),
            ContinuousVideoSegment.metadata_json["storyboard_approved"].astext == "true",
        ),
        select(
        ...
        select(
            literal("continuous_video_selected_segments").label("label"),
            func.count().label("value"),
        )
        .select_from(ContinuousVideoSegment)
        .where(
            ContinuousVideoSegment.project_id == project_id,
            ContinuousVideoSegment.metadata_json["selected_variant_asset_id"].astext.isnot(None),
        ),
...
async def project_cards() -> list[Project]:
    try:
        ...
        logger.exception("project_cards_failed")
        return []
...
async def project_summary(project_id: UUID, section: str = "script") -> dict[str, Any] | None:
    active_section = (
        section
        if section in {"script", "visual", "storyboard", "video"}
        else "script"
    )
    load_script_details = active_section == "script"
    load_visual = active_section == "visual"
    load_video = active_section in {"storyboard", "video"}
    ...
    async with AsyncSessionLocal() as session:
        ...
        if load_video:
            ...
            # Atualiza apenas prompts automáticos ainda não gerados. Assim projetos
            # existentes exibem o formato humano sem perder a proveniência dos vídeos.
            from app.video_generation.continuous import _refresh_auto_segment_prompt

            prompts_refreshed = False
            for segment in continuous_video_segments:
                prompts_refreshed = _refresh_auto_segment_prompt(segment) or prompts_refreshed
            if prompts_refreshed:
        ...
        visual_refs = [
            ...
            and (reference.target_kind != "character" or reference.view_type == "full_body")
        ]
        ...
        continuous_asset_ids.update(
            UUID(str(item["asset_id"]))
            for segment in continuous_video_segments
            for item in list((segment.metadata_json or {}).get("variants") or [])
        ...
        visual_exclusions = (
            await load_visual_exclusions(session, project_id) if load_visual else []
        )
