...
SCRIPT_DURATION_OPTIONS = list(range(1, 21))
...
IDEA_COUNT_OPTIONS = [3, 6, 9]
BLOCKING_DIALOG_PROPS = "persistent no-esc-dismiss no-backdrop-dismiss"
# A geração de roteiro nos modelos cloud leva 2-5+ min (medido: 63s-307s).
# A UI não pode declarar timeout antes do modelo responder.
UI_GENERATION_TIMEOUT_SECONDS = 720
...
STEP_LOADING_COPY = {
    ...
    "visual": (
        ...
        "A IA está criando personagens e locais para revisão.",
    ),
    "storyboard": ("Gerando storyboard", "A IA está criando quadros, planos e animatic."),
    "video": (
        "Preparando vídeo",
        "A IA está verificando prompts e frames para o pacote manual.",
    ),
}
...
ACTION_LOADING_TITLES = {
    ...
    "generate_video": "Preparando pacote",
}
...
def _missing_items_for_step(step_key: str, counts: dict[str, Any]) -> list[str]:
    ...
    visual_refs = _safe_count(
        counts, "visual_refs_ready" if "visual_refs_ready" in counts else "visual_refs"
    )
...
def _progress_for_step(step_key: str, counts: dict[str, Any]) -> tuple[int, int, str]:
    ...
    if step_key == "visual":
        ...
        if expected_refs > 0:
            ready_key = "visual_refs_ready" if "visual_refs_ready" in counts else "visual_refs"
            return min(_safe_count(counts, ready_key), expected_refs), expected_refs, "imagem"
...
def clean_idea_title(value: object, fallback: str = "História sem título") -> str:
    from app.core.title_case import clean_idea_title as _clean
...
_PT_TITLE_MINOR_WORDS = frozenset(
    {
...
_PT_TITLE_KEEP_UPPER = frozenset({"uf", "tv", "ovni", "pdf", "kg", "km", "mm", "cm"})


def standardize_title_case(value: object, fallback: str = "") -> str:
    """Padroniza o caso de um título (implementação canônica em app.core.title_case)."""
    from app.core.title_case import standardize_title_case as _standardize

    return _standardize(value, fallback)
...
(() => {
  try {
    ...
    // Pico 0.4: ~3x mais audível que o 0.135 original; os três osciladores
    // somam no mesmo GainNode com decay escalonado, então a soma permanece
    // abaixo de 1.0 (sem clipping/distorção).
