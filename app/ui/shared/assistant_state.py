...
def safe_client_navigation(client: Any, target: str | None = None) -> None:
    try:
        ...
        if target:
            ...
            # ARQ-04: history.go(0) preserva posição de scroll e diálogos;
            # um client.navigate(None) recarregaria do zero. O run_javascript
            # é executado no browser do usuário sem recriar a conexão da página.
