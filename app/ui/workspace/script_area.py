...
from app.jobs.service import cancel_generation_job, enqueue_project_step
...
# reativa o polling antes do backend commitar o script. Limitada a um número
# máximo de entradas para não crescer indefinidamente com project_ids.
_completion_sound_played: dict[str, bool] = {}
_COMPLETION_SOUND_MAX_ENTRIES = 200
...
async def _update_script_generation_progress(
    ...
    if terminal and hasattr(loading_dialog, "close"):
        ...
        if completed >= total and not _completion_sound_played.get(project_key):
            ...
            if len(_completion_sound_played) > _COMPLETION_SOUND_MAX_ENTRIES:
                # Remove as entradas mais antigas para não crescer sem limite.
                for stale_key in list(_completion_sound_played)[
                    : len(_completion_sound_played) - _COMPLETION_SOUND_MAX_ENTRIES
                ]:
                    _completion_sound_played.pop(stale_key, None)
            play_completion_sound()
        ui.navigate.reload()
        return
    ui.timer(
    ...
    *,
    ...
    if generation_in_progress:
        ...
        async def cancel_script_generation() -> None:
            ...
            raw_job_id = ai_action.get("job_id")
            if raw_job_id:
                async with AsyncSessionLocal() as session:
                    await cancel_generation_job(session, UUID(str(raw_job_id)))
            safe_close_ui_element(loading_dialog)
    ...
    with ui.row().classes("w-full gap-4 items-start"):
        with ui.column().classes("flex-1 gap-4"):
            ...
            with ui.element("div").classes("entity-card rounded-2xl p-7 min-h-[640px] w-full"):
                ...
                if script:
                    content = script.content
                elif generation_in_progress and ai_action_name == "ideas":
                    content = (
                        "A IA está criando a ideia narrativa que servirá de base para o roteiro."
                    )
                elif generation_in_progress:
                    content = "A IA está desenvolvendo o roteiro com base na ideia do projeto."
                elif ai_status == "cancelled":
                    content = "A geração foi cancelada. Use Gerar roteiro quando quiser retomar."
                elif ai_status == "failed":
                    content = "A geração não foi concluída. Revise o erro acima e tente novamente."
                else:
                    content = "Ainda não existe um roteiro. Use Gerar roteiro para iniciar."
