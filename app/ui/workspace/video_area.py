...
def render_video_area(
    ...
    *,
    ...
    _approve_continuous_video_segment_from_ui = deps._approve_continuous_video_segment_from_ui
    ...
    _select_continuous_video_variant_from_ui = deps._select_continuous_video_variant_from_ui
    ...
    try:
        # Fora de um request de página, ui.context.client lança AssertionError;
        # Client.auto_index_client não existe mais no NiceGUI 3.x (AttributeError
        # se acessado). Ambos os casos degradam para client=None, e os helpers
        # de geração seguem sem watcher de UI.
    ...
    video_generate_dialog, _video_generate_progress_callback = generation_progress_dialog(
        ...
        1,
        "vídeo",
        "Preparando a geração. Isso pode levar alguns minutos.",
        allow_background=True,
    )
    initial_frame_dialog, _initial_frame_progress_callback = generation_progress_dialog(
        "Gerando frame inicial",
        1,
        "frame",
        "Agora: criando a âncora visual do primeiro segmento.",
    )
    ...
    with ui.row().classes("w-full items-start justify-between gap-4 mb-2 flex-wrap lg:flex-nowrap"):
        with ui.column().classes("gap-1 min-w-0 flex-1"):
            ui.label("Vídeo").classes("brand-type text-3xl font-bold")
            ui.label(
                "Gere quatro opções por segmento, escolha a melhor e continue automaticamente "
                "a partir do frame extraído do vídeo escolhido."
        ...
        if continuous_segments:
            ...
            with ui.grid().classes("w-full grid-cols-1 xl:grid-cols-2 gap-4 mt-4"):
                for segment in continuous_segments:
                    ...
                    selected_variant_id = str(metadata.get("selected_variant_asset_id") or "")
                    names = [
                    ...
                    final_frame_url = _asset_content_url(
                        metadata.get("extracted_last_frame_asset_id")
                    )
                    ...
                    with (
                        ...
                        with ui.column().classes("w-full gap-1 shrink-0"):
                            ...
                            ui.label(
                                "Prompts separados por mídia"
                                if segment.segment_number == 1
                                else "Prompt de vídeo do segmento"
                            ).classes(
                                "text-sm text-[#8d938e]"
                            )
                        with ui.column().classes(
                            ...
                            initial_prompt_input: Any | None = None
                            if segment.segment_number == 1:
                                initial_prompt_input = ui.textarea(
                                    "Prompt do primeiro frame",
                                ...
                                ui.label(
                                    "Usado somente para criar a imagem que inicia o primeiro vídeo."
                                ).classes("text-xs text-[#8d938e] -mt-3")
                            segment_prompt_input = (
                                ...
                                .classes("storyboard-prompt-textarea w-full min-h-[220px]")
                            )
                            ui.label(
                                "Usado no Vibes junto com o frame inicial; descreve movimento, "
                                "câmera e evolução temporal."
                            ).classes("text-xs text-[#8d938e] -mt-3")

                        async def save_segment_prompt(
                            segment_id: UUID = segment.id,
                            flow_input: Any = segment_prompt_input,
                            frame_input: Any | None = initial_prompt_input,
                            ...
                            await _save_continuous_video_segment_prompts_from_ui(
                                ...
                                initial_frame_prompt=(
                                    str(frame_input.value or "").strip()
                    ...
                    with (
                        ...
                        with ui.row().classes("w-full items-center justify-between gap-3 shrink-0"):
                            with ui.column().classes("gap-1 min-w-0"):
                                ...
                                ui.label("Frame inicial e continuidade extraída").classes(
                                    "text-xs text-[#8d938e]"
                                )
                            ui.button(
                                icon="close",
                        ...
                        with ui.grid().classes(
                            ...
                            with ui.column().classes("w-full min-h-0 gap-2"):
                                if final_frame_url:
                                    _render_continuous_media_preview(
                                        "Frame de continuidade extraído",
                                        final_frame_url,
                                        media_kind="image",
                                        large=True,
                    ...
                    with ui.element("div").classes("entity-card rounded-2xl p-4"):
                        with ui.row().classes("w-full items-start justify-between gap-3"):
                            ...
                            status_label = (
                                "Escolha uma opção"
                                if variants and not selected_variant_id
                                else continuous_video_status_label(status_value) or "pendente"
                            )
                            ui.badge(status_label).classes(status_badge_classes)
                        if variants:
                            ui.label("Escolha a melhor opção para liberar o próximo segmento").classes(
                                "text-sm font-semibold mt-3"
                            )
                            with ui.grid().classes("w-full grid-cols-1 md:grid-cols-2 gap-3 mt-2"):
                                for option_index, variant in enumerate(variants, 1):
                                    variant_asset_id = str(variant.get("asset_id") or "")
                                    if not variant_asset_id:
                                        continue
                                    is_selected = variant_asset_id == selected_variant_id
                                    with ui.card().classes(
                                        "p-2 gap-2 bg-[#0b0e0c] border "
                                        + (
                                            "border-[#d9f05c]"
                                            if is_selected
                                            else "border-[#343934]"
                                        ...
                                        ui.video(
                                            _asset_content_url(variant_asset_id), controls=True
                                        ).classes("w-full aspect-[9/16] bg-black")
                                        with ui.row().classes(
                                            "w-full items-center justify-between gap-2"
                                        ):
                                            variant_number = int(
                                                variant.get("variant_number") or option_index
                                            )
                                            ui.label(f"Opção {variant_number}").classes(
                                                "text-sm font-medium"
                                            )
                                            if is_selected:
                                                ui.badge("Selecionada").classes(
                                                ...
                                                ui.button(
                                                    "Escolher",
                                                    icon="check_circle",
                                                    on_click=lambda asset_id=variant_asset_id, segment_id=segment.id: (
                                                        _select_continuous_video_variant_from_ui(
                                                            project_id,
                                                            segment_id,
                                                            UUID(asset_id),
                                                        )
                        ...
                        if not variants and (video_url or final_frame_url or initial_frame_url):
                        ...
                        if is_preparing_segment:
                            with ui.row().classes("items-center gap-2 mt-3 text-sm text-[#d1d4d1]"):
                                ...
                                downloaded_variant_count = int(
                                    metadata.get("video_downloaded_variant_count") or 0
                                )
                                if downloaded_variant_count:
                                    ui.label(
                                        "Gerando vídeo: "
                                        f"{downloaded_variant_count} de 4 opção(ões) já extraída(s)."
                                    )
                                else:
                                    ui.label("Gerando as 4 opções de vídeo no Vibes.")
                        if visual_summary:
                        ...
                        if qa_result is not None:
                            with ui.row().classes("items-center gap-2 mt-2 flex-wrap"):
                                ...
                                ui.badge(
                                    "Revisão humana"
                                    if qa_result.needs_human_review
                                    else "Falha objetiva"
                                ).classes("blue-status-badge bg-[#26301f]")
                        ...
                        with ui.row().classes("w-full flex-wrap justify-end gap-2 mt-2"):
                            ...
                            if status_value in {"ready", "ready_for_review"} and selected_variant_id:
                                ui.button(
                                    "Aprovar",
                                    icon="check",
                                    on_click=lambda segment_id=segment.id: (
                            ...
                            can_generate_video = (
                                ...
                                and has_initial_frame
                            )
                            if (
                                not has_initial_frame
                                ...
                                ui.button(
                                    ...
                                    "acid-bg rounded-xl min-w-[130px]"
                                )
                            elif can_generate_video:
                                ui.button(
                                    "Gerar vídeo",
                                    icon="movie_creation",
                                    on_click=lambda segment_id=segment.id: (
                                ...
                                ui.label(
                                    f"Escolha uma opção do Segmento {prev_num:02d} para obter o frame inicial"
                            ...
                            def _confirm_delete_segment(segment_id: UUID = segment.id) -> None:
                                with ui.dialog().classes("rounded-2xl") as delete_dialog:
                                    with ui.card().classes("rounded-2xl p-6 gap-4"):
                                        ...
                                        ui.label(
                                            "O segmento e seus vídeos/frames serão removidos. "
                                            "Os demais segmentos são mantidos; o segmento "
                                            "seguinte conserva o frame inicial que já tem."
                                        ).classes("text-sm text-[#8d938e]")
                                        with ui.row().classes("w-full justify-end gap-2 mt-2"):
                                            ui.button(
                                                "Cancelar",
