...
class BriefingCreate(BaseModel):
    ...
    desired_duration_minutes: Decimal = Field(ge=Decimal("2.0"), le=Decimal("25.0"))
...
class StoryHooksRequest(BaseModel):
    story_idea_id: UUID


class StoryHookRead(BaseModel):
    title: str
    description: str


class StoryHooksRead(BaseModel):
    hooks: list[StoryHookRead]


class GenerateScriptRequest(BaseModel):
    story_idea_id: UUID
    target_duration_minutes: Decimal | None = Field(
        default=None,
        ge=Decimal("1.0"),
        le=Decimal("20.0"),
    )
