...
STORY_IDEA_LANGUAGE_TEXT_FIELDS = (
    "title",
    "genre",
    "primary_emotion",
    "theme",
...
NON_PORTUGUESE_LANGUAGE_MARKERS = {
    " the ",
    " and ",
    " with ",
    " without ",
...
def _normalize_language_probe_text(value: object) -> str:
...
def story_idea_validation_errors(payload: dict) -> list[str]:
    ...
    errors.extend(_story_idea_language_errors(payload))
...
def _normalize_generated_story_ideas(content: dict, default_duration_minutes: float) -> list[dict]:
    items: list[dict] = []
    errors: list[str] = []
    raw_ideas_value: object = content.get("ideas")
    if not isinstance(raw_ideas_value, list) or not raw_ideas_value:
        # Alguns modelos obedecem ao schema narrativo, mas removem o envelope
        # ``ideas`` quando retornam somente uma ideia. Aceitar esse formato aqui
        # e manter toda a validação de campos abaixo evita descartar uma resposta
        # completa apenas por uma diferença estrutural superficial.
        for alias in ("story_ideas", "storyIdeas", "stories"):
            candidate = content.get(alias)
            if isinstance(candidate, list) and candidate:
                raw_ideas_value = candidate
                break
        else:
            nested_idea = content.get("idea")
            if isinstance(nested_idea, dict):
                raw_ideas_value = [nested_idea]
            elif any(field in content for field in STORY_IDEA_REQUIRED_TEXT_FIELDS):
                raw_ideas_value = [content]
            else:
                raw_ideas_value = _required_list(content, "ideas", "generate_story_ideas")
    raw_ideas = raw_ideas_value
