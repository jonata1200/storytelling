...
def _bounded_str_or_empty(payload: dict, key: str, context: str, max_length: int) -> str:
    """Igual a `_bounded_required_str`, mas aceita campo ausente/vazio como "".

    Campos de vídeo opcionais por design: desde `vibes_shot_v11`, a decupagem
    emite `camera_movement` vazio quando não há movimento específico e o
    compilador simplesmente OMITE a linha de câmera — vazio é um valor válido,
    não saída quebrada do modelo.
    """
    value = payload.get(key)
