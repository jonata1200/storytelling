...
@router.post("/{project_id}/script/hooks", response_model=StoryHooksRead)
async def post_script_hooks(
    project_id: UUID,
    payload: StoryHooksRequest,
    session: Annotated[AsyncSession, Depends(get_session)],
) -> StoryHooksRead:
    try:
        hooks = await generate_story_hooks(session, project_id, payload.story_idea_id)
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(exc),
    ...
    return StoryHooksRead(
        hooks=[{"title": h["title"], "description": h["description"]} for h in hooks]
    )
...
async def post_generate_script(
    ...
    try:
        script = await generate_script(
            session,
            project_id,
            payload.story_idea_id,
            payload.target_duration_minutes,
            story_hook=payload.story_hook,
        )
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
