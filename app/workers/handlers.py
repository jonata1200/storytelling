...
async def handle_video_job(job: GenerationJob) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as session:
        ...
        qa_decision = await create_or_get_media_job(
            ...
            model=get_settings().ollama_cloud_vision_model,
            attempt_key=str(job.request_payload.get("attempt_key") or "initial"),
            max_attempts=MEDIA_QA_MAX_ATTEMPTS,
        )
...
async def handle_qa_job(job: GenerationJob) -> dict[str, Any]:
    ...
    async with AsyncSessionLocal() as session:
        ...
        assessment = await analyze_with_ollama_cloud(spec, frames)
        ...
        await session.commit()
        # ARQ-02: frames de QA já cumpriram o papel — o diretório `.qa/<segment>`
        # é regenerável a qualquer momento via FFmpeg e não deve se acumular.
        ...
        if (
            ...
            decision = await create_or_get_media_job(
                ...
                max_attempts=max(source_job.max_attempts, MEDIA_MAX_ATTEMPTS),
            )
