    ...
    try:
        for view_type in wanted:
            ...
            decision = await enqueue_visual_reference_generation(
                ...
                attempt_key=f"generate-{generation_batch}-{view_type}",
            )
            jobs.append(JobRead.model_validate(decision.job))
...
async def post_regenerate_reference(
    ...
    try:
        decision = await enqueue_visual_reference_generation(
            ...
            attempt_key=f"regenerate-{reference_id}-{original.updated_at.isoformat()}",
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except (RuntimeError, TimeoutError) as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
    return JobRead.model_validate(decision.job)
