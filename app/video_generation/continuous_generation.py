...
def _resolve_variant_path(value: object) -> Path:
    """Resolve caminho de variante para Path absoluto, tipado explicitamente."""
...
async def _submit_video_job(
    ...
    if segment.generation_job_id is not None:
        ...
        if generation_job is not None:
            ...
            response_payload["project_url"] = job.project_url
    ...
    metadata["video_project_url"] = job.project_url
    metadata["video_unsigned_urls"] = list(job.unsigned_urls)
...
async def _build_video_request(
    ...
    Todo segmento usa um frame inicial. A partir do segundo, ele vem do vídeo
    escolhido no segmento anterior.
    """
    settings = get_settings()
    ...
    # O fluxo rápido usa apenas o frame inicial como âncora de movimento.
    ...
    return VideoGenerationRequest(
        ...
        prompt=video_provider_prompt(
            str(segment.prompt or "").strip(), dict(segment.metadata_json or {})
        ),
        ...
        # Todos os vídeos do projeto compartilham o MESMO projeto Vibes.
...
async def _finalize_segment_video(
    ...
    await asyncio.to_thread(normalize_video_duration, result.file_path, 8)
    normalized_content = await asyncio.to_thread(result.file_path.read_bytes)
    result = result.model_copy(
        update={"sha256": hashlib.sha256(normalized_content).hexdigest()}
    )
    ...
    existing_metadata = dict(segment.metadata_json or {})
    existing_variants = list(existing_metadata.get("variants") or [])
    first_variant_number = len(existing_variants) + 1
    asset = Asset(
        ...
        metadata_json={
            ...
            "variant_number": first_variant_number,
            "technical_role": "continuous_video_variant",
        },
    )
    ...
    variant_assets = [asset]
    variant_import_errors: list[dict[str, str | int]] = []
    for variant_index, raw_path in enumerate(
        list(getattr(job, "unsigned_urls", []))[1:], first_variant_number + 1
    ):
        try:
            source_path = await asyncio.to_thread(
                _resolve_variant_path, raw_path
            )
            destination = output_dir / f"vibes-{job.id}-variant-{variant_index}.mp4"
            await asyncio.to_thread(shutil.copyfile, source_path, destination)
            await asyncio.to_thread(normalize_video_duration, destination, 8)
            content = await asyncio.to_thread(destination.read_bytes)
            if len(content) < 12 or content[4:8] != b"ftyp":
                raise ValueError("arquivo vazio, corrompido ou incompatível")
        except (OSError, ValueError) as exc:
            logger.warning(
                "video_variant_import_failed segment_id=%s variant=%s path=%s error=%s",
                segment.id,
                variant_index,
                raw_path,
                exc,
            )
            variant_import_errors.append(
                {"variant_number": variant_index, "path": str(raw_path), "error": str(exc)}
            )
            continue
        variant_metadata = {
        ...
        variant_asset = Asset(
            ...
            artifact_id=None,
            kind=AssetKind.VIDEO,
            name=f"Vídeo Segmento {segment.segment_number:03d} — Opção {variant_index}",
            storage_uri=destination.as_posix(),
            content_type="video/mp4",
            sha256=hashlib.sha256(content).hexdigest(),
            metadata_json=variant_metadata,
        )
        apply_asset_storage_metadata(variant_asset)
        ...
        session.add(
            AssetVersion(
                ...
                version_number=1,
                storage_uri=variant_asset.storage_uri,
                sha256=variant_asset.sha256,
                metadata_json=variant_metadata,
            )
        )
        variant_assets.append(variant_asset)
        ...
        *[
            {
                ...
                "variant_number": first_variant_number + index - 1,
                "provider": result.provider,
                "model": model,
                "created_at": metadata["video_completed_at"],
                "batch_number": (first_variant_number - 1) // 4 + 1,
            }
            for index, item in enumerate(variant_assets, 1)
        ],
    ]
    metadata.pop("selected_variant_asset_id", None)
    metadata.pop("selected_variant_storage_uri", None)
    metadata.pop("extracted_last_frame_asset_id", None)
    metadata["awaiting_variant_selection"] = True
    metadata["video_variant_count"] = len(variant_assets)
    if variant_import_errors:
        metadata["video_variant_import_errors"] = variant_import_errors
    else:
        metadata.pop("video_variant_import_errors", None)
    ...
    await _emit_continuous_video_segment_event(
        ...
        provider=result.provider,
        model=model,
        message=f"Opções do Segmento {segment.segment_number:03d} prontas para escolha.",
        estimated_cost=total_cost,
        details={
            "video_asset_id": str(asset.id),
            "variant_count": len(variant_assets),
            "variant_import_errors": variant_import_errors,
            "awaiting_variant_selection": True,
        },
    )
    await session.flush()
...
async def generate_video_for_segment(
    ...
    try:
        ...
        if segment.source_frame_asset_id is None:
            raise ValueError("Gere ou herde o frame inicial antes de criar os vídeos.")
        existing_job_id = str(
        ...
        job = VideoJob(
            ...
            project_url=str((segment.metadata_json or {}).get("video_project_url", "")),
            status="pending",
            provider=str(segment.provider or _resolve_video_provider()),
            model=str(segment.model or ""),
            prompt=str(segment.prompt or ""),
            unsigned_urls=list((segment.metadata_json or {}).get("video_unsigned_urls") or []),
        )
        ...
        while True:
            ...
            try:
                ...
                poll_count += 1
                metadata = dict(segment.metadata_json or {})
                metadata["video_last_poll_error"] = str(poll_exc)
                metadata["video_last_poll_error_at"] = datetime.now(UTC).isoformat()
                segment.metadata_json = metadata
                await _emit_continuous_video_segment_event(
                    session,
                    segment=segment,
                    ...
                    model=str(segment.model or ""),
                    message=(
                        "Falha transitória ao consultar o Vibes; "
                        "uma nova tentativa será feita."
                    ),
                    details={"poll_attempt": poll_count, "error": str(poll_exc)},
                )
                await session.commit()
                continue
            poll_count += 1
            await _emit_continuous_video_segment_event(
                ...
                model=str(segment.model or ""),
                message=(
                    f"Vibes retornou {job_update.status}: "
                    f"{len(job_update.unsigned_urls)} de 4 opções baixadas."
                ),
                details={
            ...
            if job_update.project_url:
                job.project_url = job_update.project_url
            if job_update.usage_cost:
                try:
                    usage_cost = Decimal(str(job_update.usage_cost))
                except (InvalidOperation, ValueError):
                    usage_cost = None

            metadata = dict(segment.metadata_json or {})
            metadata["video_unsigned_urls"] = list(job.unsigned_urls)
            metadata["video_downloaded_variant_count"] = len(job.unsigned_urls)
            metadata["video_project_url"] = job.project_url
            metadata["video_last_poll_at"] = datetime.now(UTC).isoformat()
            metadata.pop("video_last_poll_error", None)
            metadata.pop("video_last_poll_error_at", None)
            segment.metadata_json = metadata
            if segment.generation_job_id is not None:
                generation_job = await session.get(GenerationJob, segment.generation_job_id)
                if generation_job is not None:
                    response_payload = dict(generation_job.response_payload or {})
                    response_payload["project_url"] = job.project_url
                    response_payload["downloaded_variant_count"] = len(job.unsigned_urls)
                    response_payload["unsigned_urls"] = list(job.unsigned_urls)
                    generation_job.response_payload = response_payload
            await session.commit()
        ...
        await _finalize_segment_video(session, project_id, segment, job, usage_cost)
