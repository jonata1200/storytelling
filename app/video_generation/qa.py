...
async def analyze_with_meta(
    spec: ShotGenerationSpec, frames: list[Path]
) -> QAAssessment:
    """Compara frames com o contrato do Shot usando a API multimodal oficial configurada."""
    settings = get_settings()
    if not settings.qa_meta_multimodal_enabled:
        return conservative_qa_assessment(spec)
    base_url = str(settings.meta_base_url or "").rstrip("/")
    if not base_url:
        raise RuntimeError("META_BASE_URL não configurada para QA multimodal")
    ...
    payload = json.dumps({
        "model": settings.meta_default_model,
    ...
    def request() -> QAAssessment:
        ...
        try:
            with urllib.request.urlopen(req, timeout=settings.qa_meta_timeout_seconds) as response:
                body = json.loads(response.read().decode("utf-8"))
            raw = body["choices"][0]["message"]["content"]
            return QAAssessment.model_validate_json(raw)
        except Exception as exc:
            raise RuntimeError(f"Falha no QA multimodal Meta: {redact_secrets(exc)}") from exc

    return await asyncio.to_thread(request)


def conservative_qa_assessment(spec: ShotGenerationSpec) -> QAAssessment:
    ...
    return QAAssessment(
        ...
        reasons=["QA multimodal desativado; revisão humana obrigatória"],
    )
