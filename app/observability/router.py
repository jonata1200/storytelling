...
@router.get("/provider-health", response_model=list[ProviderChannelHealthRead])
async def get_provider_channel_health(
    session: Annotated[AsyncSession, Depends(get_session)],
    limit: Annotated[int, Query(ge=1, le=100)] = 10,
) -> list[ProviderChannelHealthRead]:
    """Últimas N gerações por canal (meta/vibes) com contagem de falhas (ARQ-03)."""
    return await provider_channel_health(session, limit=limit)
